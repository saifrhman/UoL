...
\begin{itemize}
  \item \textbf{No assumption about cluster shape.}  Unlike $K$-means
        ...
        exhibits strong size imbalance: Cluster~2 (Shakespeare, 790 documents)
        and Cluster~3 (Wikipedia, 90 documents).
...
distances in the 100-dimensional PCA space weight the earlier, higher-variance
...
\begin{tabular}{>{\centering\arraybackslash}p{2cm}
  ...
  2  & 0.084815 & \\
  \rowcolor{greenbg}
  3  & \textbf{0.091923} & \textbf{Global optimum (peak)} \\
  4  & 0.070843 & \\
  5  & 0.045152 & \\
  6  & 0.035833 & \\
  7  & 0.039736 & \\
...
number of clusters, with a peak coefficient of $\mathbf{0.091923}$.  The score
at $K=3$ exceeds that at $K=2$ (0.084815) and all higher values of $K$, which
decrease to a minimum of $0.035833$ at $K=6$ before gradually recovering
from $K=7$ onwards.  The shape of the curve --- a clear peak at $K=3$ followed by
...
the 100-dimensional PCA feature space.
...
\begin{tabularx}{\textwidth}{>{\centering\bfseries}p{1.5cm}
  ...
  2 & 790 (44.9\%) & Shakespeare (Coriolanus) &
...
the next candidate; however, its silhouette score of $0.070843$ --- a 23\% drop
...
datasets.  The score of $0.091923$ achieved here falls comfortably within this
range --- above BERT + $k$-means on the 20Newsgroups dataset
($\mathrm{SS} = 0.048$) and consistent with reported scores for AHC on
...
Cluster~3 (Wikipedia) contains only 90 documents ($\approx 5.1\%$ of the
